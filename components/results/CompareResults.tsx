    ...
    <div className="compare-shell">
      <div className="compare-head">
        <div>
          ...
          <p className="subtle">
            {truthful
              ? "Real captures and harness results only — scores, latency, and output come from stored runs."
              : "Sample demo data — not your product traffic."}
            {source === "live" ? " Source: live SQLite runs." : null}
            {source === "harness" ? " Source: posted harness results." : null}
          </p>
          <div className="status-row" style={{ marginTop: 8 }}>
            <span className={`tag ${truthful ? "ok" : "warn"}`}>{truthful ? "truthful" : "sample"}</span>
            <span className="tag blue">{source}</span>
            <span className="tag">{report.rows.length} rows</span>
          </div>
        </div>
      ...
      {isEmpty ? (
        <div className="empty-state" style={{ padding: 28 }}>
          <strong>{report.emptyReason || "No results yet"}</strong>
          <span className="subtle">
            Dashboard → Import this session, or POST real harness results to /api/compare.
          </span>
        </div>
      ) : (
        <>
                    ...
                    return (
                      <th key={version}>
                        <div className="compare-version-head">
                          ...
                          {summary ? (
                            <span
                              className={`tag ${summary.passRatio >= 80 ? "ok" : summary.passRatio >= 50 ? "warn" : "danger"}`}
                            >
  ...
  return (
    <div className={`compare-cell ${cell.passed ? "pass" : "fail"}`}>
      ...
      <div className="compare-metrics">
        ...
        <span>Cost {cell.costUsd > 0 ? `$${cell.costUsd.toFixed(4)}` : "—"}</span>
        {cell.tokens != null ? <span>Tokens {cell.tokens}</span> : null}
        {cell.tokensPerSec != null ? <span>Tokens/Sec {cell.tokensPerSec}</span> : null}
      </div>
    </div>
  );
}
